...
namespace velodyne_rawdata
{
  ...
  velodyne_pointcloud::Calibration* RawData::setup(std::string calibration_file, std::string model)
  {
    config_.model = model;
    buildTimings();
  ...
  void RawData::unpack(const velodyne_msgs::VelodynePacket &pkt, DataContainerBase& data, const ros::Time& scan_start_time)
  {
    ...
    for (int i = 0; i < BLOCKS_PER_PACKET; i++) {
      ...
      for (int j = 0, k = 0; j < SCANS_PER_BLOCK; j++, k += RAW_SCAN_SIZE) {
        ...
        intensity += focal_slope * (std::abs(focal_offset - 256 * SQR(1 - static_cast<float>(tmp.uint)/65535)));
  ...
  void RawData::unpack_vls128(const velodyne_msgs::VelodynePacket &pkt, DataContainerBase& data, const ros::Time& scan_start_time) {
    ...
    for (int block = 0; block < BLOCKS_PER_PACKET - (4* dual_return); block++) {
      ...
      default:
        ...
        ROS_WARN_STREAM_THROTTLE(60, "skipping invalid VLS-128 packet: block " << block << " header value is " << raw->blocks[block].header);
